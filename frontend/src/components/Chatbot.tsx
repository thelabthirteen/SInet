...
const Chatbot: React.FC<ChatbotProps> = ({ user }) => {
  ...
  const generateBotResponse = (input: string) => {
    ...
    if (lowerInput.includes('graduation') || lowerInput.includes('requirement')) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">UTMKL Assistant</h1>
            <p className="text-gray-600">AI-powered help for all your academic needs</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: BookOpen, title: 'Course Info', desc: 'Syllabi, schedules, prerequisites' },
            { icon: Calendar, title: 'Deadlines', desc: 'Assignments, exams, submissions' },
            { icon: HelpCircle, title: 'Guidelines', desc: 'Academic policies & procedures' },
            { icon: Sparkles, title: 'Smart Help', desc: 'Trained on UTMKL resources' }
          ].map((feature, index) => {
